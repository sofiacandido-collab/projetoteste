import streamlit as st

st.title('Vencedores do Oscar 2026')
st.write("Essa página reúne todos os vencedores dos Oscars 2026")

st.image("https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRxZxR57Uvr_NcwcJQFpG7jNeWGav36sy6w-g&s")

st.write("-----------------------------------------")
st.markdown('Melhor Filme')
st.markdown("""Indicados: 
- O Agente Secreto 
- Uma Batalha Após a Outra 
- Bugonia
- F1: O Filme
- Frankenstein
- Hamnet
- Pecadores 
- Marty Supreme 
- Valor Sentimental
- Sonhos de Trem 
""")

st.write("Vencedor: Uma Batalha Aós a Outra")
st.markdown("""Sobre esse filme: 
- Título original: One Battle After Another
- Diretor: Paul Thomas Anderson
- Roteiro: Paul Thomas Anderson
- Elenco principal: Leonardo DiCaprio, Sean Penn, Regina Hall
- Data de lançamento: Previsto para 2025
- Gênero: Drama / Crime
- Inspiração: Baseado no romance Vineland, de Thomas Pynchon
- Ambientação: Estados Unidos, com contexto político e social
- Produção: Warner Bros. Pictures
- Tema central: Conflitos pessoais e políticos, com foco em personagens lidando com sistemas de poder e resistência
